...
class Video_publisher(Node):
    ...
    def __init__(self):
        ...
        self.listener_mov = self.create_subscription(String,'/movement',self.listener_mov_callback,10)
        self.listener_mov
        self.listener_takeoff = self.create_subsription(Twist,"/takeoff", self.listener_takeoff_callback,10)
        self.listener_takeoff

        timer_period = 0.5  # seconds
        self.timer = self.create_timer(timer_period, self.timer_callback)
        self.timer = self.create_timer(timer_period,self.listener_mov_callback)
        self.timer = self.create_timer(timer_period, self.listener_takeoff_callback)
        self.i = 0
        

    def listener_mov_callback(self, mov_msg):
        self.drone.set_roll(mov_msg.angular.x)
        self.drone.set_pitch(mov_msg.angular.y)
        self.drone.set_yaw(mov_msg.angular.z)
        self.drone.set_throttle(mov_msg.linear.z)

    def listener_takeoff_callback(self, takeoff_msg):
        if takeoff_msg == "q":
            self.drone.land()
